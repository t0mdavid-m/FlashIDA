            ...
            if (ms2Mzs != null && targets.Count > 0)
            {
                ...
                if (ms2PeakGroups > 0)
                {
                    ...
                    if (ms3Config != null && IsActive(ms3Config.Active))
                    {
                        ...
                        // Fragment matching against protein sequence (requires OPENMS_DATA_PATH)
                        if (!String.IsNullOrEmpty(ms3Config.MS3ProteinSequence))
                        {
                            ...
                            foreach (var fm in fragMatches)
                            {
                                Console.WriteLine("[MS3-FRAG-TARGET] mass={0:F4} charge={1} ion={2}",
                                    fm.Mass, fm.Charge, fm.IonName ?? "?");
                            }
                        }
                    }
                }
