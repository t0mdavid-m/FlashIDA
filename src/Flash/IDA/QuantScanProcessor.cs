    ...
    public class QuantScanProcessor : IScanProcessor
    {
        ...
        public IEnumerable<IFusionCustomScan> ProcessMS(IMsScan msScan)
        {
            ...
            if (methodParams.UseUnifiedBridge)
            {
                ...
                string scanDescStr = msScan.Header["Scan"] ?? "";
                flashIdaWrapper.ProcessScan(mzs, ints, rt, msLevel, scanDescStr);
                return scans;
            }
            ...
            if (msScan.Header["MSOrder"] == "1" && msScan.Header["MassAnalyzer"] == "FTMS")
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        IDAlog.Debug(String.Format("[SHADOW] ProcessScan returned {0} for quant scan {1}", shadowResult, msScan.Header["Scan"]));
                    }
