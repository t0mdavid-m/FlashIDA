...
- **CI run:** `23741455500` on `phase-4` branch (B2 re-capture)
...
- **Note:** B2 verification confirmed all modes exercise their specific logic:
  inclusion (monoisotopic targets within 20 ppm), strict inclusion (target-only),
  deep (TargetLog deprioritization), exclusion (TargetLog mass exclusion),
  MS3 (GetBestMS2Masses + GetTopFragmentMatches with CytC fragment ions).
